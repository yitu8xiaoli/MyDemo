...
@implementation CompleteOrderView
...
-(void)setUI
{
    ...
    [self.orderBackImage addSubview:self.transferLab];
...
-(UIImageView *)orderBackImage
{
    if (_orderBackImage == nil) {
        self.orderBackImage =[[UIImageView alloc]initWithFrame:CGRectMake(15, 69, kScreenW-30, 348)];
...
-(UIButton *)complaintBtn
{
    if (_complaintBtn == nil) {
        ...
        [self.complaintBtn setTitle:@"立即划转" forState:UIControlStateNormal];
        self.complaintBtn.titleLabel.font = [UIFont systemFontOfSize:18];
        self.complaintBtn.backgroundColor = RGB(4, 197, 252, 1);
...
{
    if (_transferLab == nil) {
        _transferLab = [[UILabel alloc] initWithFrame:CGRectMake(15, maxY(self.complaintBtn)+ 7, kOrderBackImageWidth-30, 17)];
        _transferLab.font = [UIFont systemFontOfSize:12.0f];
        _transferLab.textAlignment = NSTextAlignmentCenter;
        _transferLab.textColor = RGB(10, 31, 55, 0.56);;
        _transferLab.adjustsFontSizeToFitWidth = YES;
        _transferLab.minimumScaleFactor = 0.5;
        _transferLab.text = @"法币账户与交易账户之间的划转手续费全免";
    }
    return _transferLab;
}
@end
