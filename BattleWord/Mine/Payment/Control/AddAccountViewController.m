...
@interface AddAccountViewController ()<UIImagePickerControllerDelegate,UIActionSheetDelegate,UINavigationControllerDelegate>
@property (nonatomic,strong)AddAccountView *addVC;
...
- (void)viewDidLoad {
    ...
    self.addVC = [[AddAccountView alloc]initWithFrame:CGRectMake(0, 0, kScreenW, kScreenH)];
    self.view = self.addVC;
    //添加背景图片
...
//    self.addVC.userInteractionEnabled = YES;
//    self.addVC.headView.userInteractionEnabled = YES;

    self.addVC.addImage.userInteractionEnabled = YES;

    UITapGestureRecognizer *tapGesture= [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(tapAction)];
    [self.addVC.addImage addGestureRecognizer:tapGesture];
    
    [self.addVC.nextBtn addTarget:self action:@selector(tapAction) forControlEvents:UIControlEventTouchUpInside];
...
-(void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary<NSString *,id> *)info{
    ...
    self.addVC.addImage.image = saveImage;
    NSDictionary *dic = @{
