...
@interface UserFeedbackViewController ()<UITextViewDelegate,UIImagePickerControllerDelegate,UIActionSheetDelegate,UINavigationControllerDelegate>
@property (nonatomic,strong)UserFeedbackView *userView;
...
- (void)viewDidLoad {
    ...
    self.title = @"用户反馈";
    self.userView = [[UserFeedbackView alloc]initWithFrame:[UIScreen mainScreen].bounds];
    self.view = self.userView;
    self.userView.adviceTextView.tag = 1001;
    self.userView.adviceTextView.delegate = self;
    self.userView.contactTextView.tag = 1002;
    self.userView.contactTextView.delegate = self;
...
- (void)textViewDidChange:(UITextView *)textView
{
    ...
    if (textView.tag == 1002) {
        self.userView.placeholderContactLab.hidden = YES;
    }
}
#pragma mark 创建底部弹框
