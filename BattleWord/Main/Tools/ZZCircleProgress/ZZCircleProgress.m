...
static NSInteger kPointMultiple = 9; // 圆点倍数
...
- (void)initialization {
    ...
    _pointImage = [UIImage imageNamed:@"my_icon_circle"];
...
- (void)drawRect:(CGRect)rect {
    ...
    CGFloat radius = maxWidth/2.0-_strokeWidth/2.0-10;//留出一像素，防止与边界相切的地方被切平
    ...
    //画小圆点
    if (_showPoint) {
        CGContextDrawImage(ctx, CGRectMake(CircleSelfWidth/2 + ((CGRectGetWidth(self.bounds)-_strokeWidth)/2.f-10)*cosf(valueEndA)-_strokeWidth*kPointMultiple/2.0, CircleSelfWidth/2 + ((CGRectGetWidth(self.bounds)-_strokeWidth)/2.f-10)*sinf(valueEndA)-_strokeWidth*kPointMultiple/2.0, _strokeWidth*kPointMultiple, _strokeWidth*kPointMultiple), _pointImage.CGImage);
    }
    ...
    if (_showProgressText) {
       ...
       //构建属性集合
        NSDictionary *attributes = @{NSFontAttributeName:font,NSForegroundColorAttributeName:wColor, NSParagraphStyleAttributeName:textStyle};
        //获得size
...
-(UILabel *)clearLab
{
    if (_clearLab == nil) {
        //描述
        _clearLab = [[UILabel alloc] initWithFrame:CGRectMake(83.5, 138, 38, 17)];
        _clearLab.font = [UIFont systemFontOfSize:12.0f];
        _clearLab.textColor = RGB(255, 255, 255,1.0);
        _clearLab.text = @"清除中";
    }
    return _clearLab;
}
-(UILabel *)tagLab
{
    if (_tagLab == nil) {
        //描述
        _tagLab = [[UILabel alloc] initWithFrame:CGRectMake(57.5, maxY(_clearLab)+5, 90, 17)];
        _tagLab.font = [UIFont systemFontOfSize:12.0f];
        _tagLab.textColor = RGB(255, 255, 255,1.0);
        _tagLab.textAlignment = NSTextAlignmentCenter;
        _tagLab.text = @"9.9M/12.5M";
    }
    return _tagLab;
}
//最后一次动画所做的处理
