...
- (void)initSubViews {
    ...
    self.scrollview.backgroundColor = RGB(10, 31, 55, 1);
    [self.movLine setBackgroundColor:[UIColor colorWithRed:4/255 green:197/255.0 blue:252/255.0 alpha:1]];
}
...
- (void)makeButtonsByTitles {
    ...
    for( NSString *title in _titles ) {
        ...
        [btn.titleLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            // titlelabel ～ 标题标签的约束
            make.leading.equalTo(btn).with.offset(0);
            make.trailing.equalTo(btn).with.offset(0);
        ...
        [btn mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            if (self.buttons.count == 1) {
                make.leading.equalTo(lastView.mas_trailing).with.offset(15);
            }
            else
            {
                make.leading.equalTo(lastView.mas_trailing).with.offset(self.xSpan);

            }
            if (self.buttons.count == _titles.count) {
                make.trailing.equalTo(lastView.mas_trailing).with.offset(15);
            }
