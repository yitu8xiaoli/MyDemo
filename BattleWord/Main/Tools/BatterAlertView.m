...
@implementation BatterAlertView
...
{
    if (_contentLab ==nil) {
        self.contentLab = [[UILabel alloc]initWithFrame:CGRectMake((kScreenW-30-170)/2, maxY(_titleLab)+18, 170, 48)];
        self.contentLab.font = [UIFont systemFontOfSize:17];
        self.contentLab.textAlignment = NSTextAlignmentCenter;
        self.contentLab.numberOfLines = 0;
        self.contentLab.text = @"如果您已经向卖家付款请千万不要取消交易!";
...
{
    if (_prometLab ==nil) {
        self.prometLab = [[UILabel alloc]initWithFrame:CGRectMake(15, maxY(_contentLab)+20, self.backView.frame.size.width-30, 17)];
        self.prometLab.font = [UIFont systemFontOfSize:12];
        self.prometLab.textAlignment = NSTextAlignmentCenter;
        self.prometLab.text = @"取消规则:买家当日累计3笔取消,会限制当日买入功能";
